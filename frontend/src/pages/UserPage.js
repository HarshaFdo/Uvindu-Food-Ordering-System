      ...
      <header className="sticky top-0 z-50 backdrop-blur-lg bg-white/90 border-b border-orange-200/50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3">
          ...
          <nav className="flex justify-center space-x-8 mt-4 border-t border-orange-100 pt-3">
            <button className="font-bold text-orange-500 pb-1 border-b-2 border-orange-400">Home</button>
            <button className="font-medium text-gray-600 hover:text-orange-500 transition-colors duration-200">Menu</button>
            <button onClick={() => navigate("/my-orders")} className="font-medium text-gray-600 hover:text-orange-500 transition-colors duration-200">My Orders</button>
          </nav>
        </div>
      </header>
              ...
              <div className="bg-gradient-to-br from-orange-500 to-yellow-500 rounded-2xl p-6 text-white shadow-lg">
                ...
                {activeOrders.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-white/60 text-sm">Start ordering to track your meals</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {activeOrders.map((order) => (
                      <div key={order.id} className="bg-white/15 backdrop-blur-sm rounded-xl p-4">
                        <div className="flex justify-between items-center mb-2">
                          <span className="font-semibold">Order #{order.id}</span>
                          <span className="text-sm bg-green-400/20 px-2 py-1 rounded-full">{order.status}</span>
                        </div>
                        {order.eta_minutes && (
                          <p className="text-sm text-white/80">ETA: {order.eta_minutes} minutes</p>
                        )}
                        <button onClick={() => navigate("/delivery-map")} className="w-full mt-3 bg-white/20 hover:bg-white/30 py-2 rounded-lg text-sm font-medium transition-all duration-200">
                          Track Order
                        </button>
                      </div>
                    ))}
              ...
              <div className="bg-gradient-to-br from-green-500 to-emerald-500 rounded-2xl p-6 text-white shadow-lg">
                ...
                {recentOrders.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-white/60 text-sm">Your order history will appear here</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {recentOrders.slice(0, 2).map((order) => (
                      <div key={order.id} className="bg-white/15 backdrop-blur-sm rounded-xl p-4">
                        <div className="flex justify-between items-center">
                          <div>
                            <span className="font-semibold">Order #{order.id}</span>
                            <p className="text-sm text-white/80">Rs. {order.total ?? 'N/A'}</p>
                          </div>
                          <span className="text-xs bg-green-400/20 px-2 py-1 rounded-full">{order.status}</span>
                        </div>
                      </div>
                    ))}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-3xl font-bold text-gray-800">
                  Our <span className="bg-gradient-to-r from-orange-500 to-yellow-500 bg-clip-text text-transparent">Menu</span>
                </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredMeals.length === 0 ? (
                  <div className="col-span-full text-center py-12 bg-white/60 backdrop-blur-sm rounded-2xl">
                    ...
                    <button onClick={() => setSearchText("")} className="bg-gradient-to-r from-orange-500 to-yellow-500 text-white px-6 py-2 rounded-full font-medium hover:shadow-md transition-all duration-200">
                      Clear Search
                    </button>
                  </div>
                ) : (
                ...
                <div className="p-8 text-center">
                  <div className="w-20 h-20 bg-gradient-to-br from-orange-100 to-yellow-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <svg className="w-10 h-10 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 5M7 13l2.5 5m0 0h8m-8 0L5 21m0 0l2.5-5" />
                    </svg>
                  </div>
                  <h4 className="text-xl font-bold text-gray-700 mb-2">Your cart is empty</h4>
                  <p className="text-gray-500 text-sm mb-6 leading-relaxed">
                    Browse our delicious menu and add your favorite meals to get started!
                  </p>
                  <div className="space-y-3">
